...
prediction.performance.calib=function(prediction,label,nbins){
...
#   auc=perf@y.values 
  ...
  for (k in 1:nbins){
    ...
    obs[k]=mean(label[grp])
    pred[k]=mean(prediction[grp])
    obssd=sqrt(obs[k]*(1-obs[k]))/(sqrt(length(grp)))
  }
  ...
  out=list(calib,brier,rel,res,unc,obs,pred,obssd)
  
  return(out)
}

prediction.performance.calib2=function(prediction,label,breaks){
  #   pred=prediction(prediction,label)
  #   perf=performance(pred,"auc")
  #   auc=perf@y.values 
  ...
  out=list(calib,brier,rel,res,unc,obs,pred)
